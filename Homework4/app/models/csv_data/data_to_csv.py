...
def check_directory_empty(directory, db_path, tickers):
    if not os.listdir(directory):
        # directory is empty -> create csv_data for every code
        for ticker in tickers:
            data_to_csv(directory, db_path, ticker)
    else:
        # directory is not empty -> check if code is missing and fill csv_data with data
        files = glob.glob(directory + "/*" + '.csv_data')
        for ticker in tickers:
            filename = ticker + '.csv_data'
