...
from Homework4.scraper_app.app import data_scraper_app
from Homework4.scraper_app.model import data_scraper


@data_scraper_app.route('/<string:ticker>', methods=['GET'])
def get_scraper_ticker_data(ticker):
    return jsonify(data_scraper.get_ticker_data(ticker)), 200

@data_scraper_app.route('/<string:ticker> <string:from_date> <string:to_date>', methods=['GET']) # date format YYYY-MM-DD
